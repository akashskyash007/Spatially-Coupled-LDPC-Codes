...
void construct_matrix(int num_rows_in_protograph, int num_columns_in_protograph, int expansion_factor) {
	int H_protograph[num_rows_in_protograph][num_columns_in_protograph];
	FILE* fptr;
	fptr = fopen("D_Matrix_Protograph.txt", "r");
...
int main(void) {
	...
	int num_columns_in_protograph = 2;
	int expansion_factor = 16;
	construct_matrix(num_rows_in_protograph, num_columns_in_protograph, expansion_factor);
