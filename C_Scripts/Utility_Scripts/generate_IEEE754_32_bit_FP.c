...
void generate_spfp_representation(float input, FILE* fptr) {
...
int main(void) {
	...
	for (int i = 0; i < 40; i++) {
		inputs[i] = rand() * 1.0 / 100000000;
		printf("%f\n", inputs[i]);
	}
	FILE* fptr;
	fptr = fopen("row_processing_test_inputs.txt", "w");
	for (int i = 0; i < num_inputs; i++) {
		generate_spfp_representation(inputs[i], fptr);
	}
	fclose(fptr);
	system("vim row_processing_test_inputs.txt");
	return 0;
}
